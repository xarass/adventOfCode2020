class Problem2 {
    ...
    executePartOne() {
        ...
        for (let line of lines) {
            ...
            sections[1] = sections[1].charAt(0);

            const min = sections[0].split('-')[0];
            const max = sections[0].split('-')[1];

            let counter = 0;
            for (let char of sections[2]) {
                if (char === sections[1]) {
                    counter++;
                }
            }

            // console.log("min: " + min + " max: " + max + " counter: " + counter);
            if (Number(min) <= counter && Number(max) >= counter) {
                validPasswordCount++;
            }
        }
    ...
    executePartTwo() {
        ...
        for (let line of lines) {
            ...
            const pos1 = Number(sections[0].split('-')[0]);
            const pos2 = Number(sections[0].split('-')[1]);

            const isAtPos1 = sections[2].charAt(pos1 - 1) === sections[1];
            const isAtPos2 = sections[2].charAt(pos2 - 1) === sections[1];
            console.log(isAtPos1 + " " + isAtPos2);
    ...
    private fileText = "6-10 s: snkscgszxsssscss\n" +
        "6-7 b: bbbbbxkb\n" +
        "2-4 n: nnnjn\n" +
        "1-2 j: jjjj\n" +
        "5-9 z: jgzzzqhbj\n" +
